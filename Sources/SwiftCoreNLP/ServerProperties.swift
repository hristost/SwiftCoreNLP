...
extension CoreNLPServer {
    ...
    struct Properties: Encodable {
        ...
        enum Serializer: String, Encodable {
            /// Writes the output to a protocol buffer, as defined in the definition file `edu.stanford.nlp.pipeline.CoreNLP.proto`
            case protoBuf = "edu.stanford.nlp.pipeline.ProtobufAnnotationSerializer"
            /// Writes the output to a Java serialized object. This is only suitable for transferring data between Java programs. This also produces relatively large serialized objects.
            case genericAnnotation = "edu.stanford.nlp.pipeline.GenericAnnotationSerializer"
            /// Writes the output to a (lossy!) textual representation, which is much smaller than `Serializer.generic`, but does not include all the relevant information
